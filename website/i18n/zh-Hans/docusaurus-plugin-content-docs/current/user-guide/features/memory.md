...
两个文件均存储于 `~/.hermes/memories/`，在常规会话开始时以冻结快照的形式注入系统 prompt（提示词）；Cron 会话不会预加载。Agent 通过 `memory` 工具管理自身记忆——可添加、替换或删除条目。
...
常规交互会话开始时，记忆条目从磁盘加载并以冻结块的形式渲染到系统 prompt 中。Cron 会话会有意省略 profile 级 SOUL.md、MEMORY.md 和 USER.md 上下文；它们会保留任务 prompt 和配置的工作目录上下文：
...
**冻结快照模式：** 对于加载 profile 记忆的会话，系统 prompt 注入在会话开始时捕获一次，会话中途不会改变。这是有意为之——目的是保留 LLM 的前缀缓存以提升性能。当 Agent 在会话期间添加或删除记忆条目时，更改会立即持久化到磁盘，但要到下一次常规会话开始时才会出现在系统 prompt 中。工具响应始终显示实时状态。
...
没有内置的 `read` 操作。常规会话会在系统 prompt 中收到冻结的记忆快照；Cron 会话不会预加载该快照，也不会自动预取外部记忆。启用时，配置的 memory 工具仍可供 Cron agent 显式操作。
